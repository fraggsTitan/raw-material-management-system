...
CREATE TABLE raw_materials( material_id bigserial,rest_id bigint,material_name varchar(50),
...
PRIMARY KEY(material_id),FOREIGN KEY(rest_id) REFERENCES restaurants(id),
...
CREATE TABLE contacts(supplier_id bigint,restaurant_id bigint,phone_supplier varchar(15),
FOREIGN KEY(supplier_id) REFERENCES suppliers(supplier_id), FOREIGN KEY(restaurant_id)
REFERENCES restaurants(id), PRIMARY KEY(supplier_id,restaurant_id,phone_supplier));
...
supplier_id bigint REFERENCES suppliers(supplier_id),place_date  timestamp,
...
stock_wasted int,material_id bigint REFERENCES raw_materials(material_id),
shipment_id bigint REFERENCES material_updates(shipment_id));
